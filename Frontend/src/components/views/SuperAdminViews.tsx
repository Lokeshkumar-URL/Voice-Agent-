...
function SuperAdminDashboard() {
  ...
  if (!dashboard) return (
    <div className="space-y-6" aria-label="Loading live platform data">
      <div className="zea-overview-kpi-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
  ...
  return (
    <div className="zea-company-inspect space-y-6">
      {/* Overview Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="zea-kpi-card flex flex-col bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <span className="text-xs font-bold uppercase text-slate-400 tracking-wider">Active Organizations</span>
          <div className="mt-2 text-center">
            <h4 className="font-black text-slate-800">
              <span className="zea-kpi-value block text-[2.5rem]">{overview.activeCompanies}</span>
        ...
        <div className="zea-kpi-card flex flex-col bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="mt-2 text-center">
            <h4 className="font-black text-slate-800">
              <span className="zea-kpi-value block text-[2.5rem]">{overview.inFlightCalls}</span>
        ...
        <div className="zea-kpi-card flex flex-col bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="mt-2 text-center">
            <h4 className="zea-kpi-value text-[2.5rem] font-black text-slate-800">{overview.callsToday.toLocaleString()}</h4>
          </div>
        ...
        <div className="zea-kpi-card flex flex-col bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="mt-2 text-center">
            <h4 className="zea-kpi-value text-[2.5rem] font-black text-slate-800">₹{overview.monthlyRevenue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h4>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <CallVolumeChart data={callTraffic} className="zea-global-call-traffic-chart" />
        </div>

        <div className="zea-voice-output-panel bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
          ...
          <OutcomePieChart data={outcomes} className="zea-voice-output-chart" />
        </div>
      </div>
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm relative">
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-[10px] pb-3">
              ...
              <th className="pb-3 text-right">Assigned Numbers</th>
              ...
              <tr key={c.id} className="hover:bg-slate-50/50 group">
                ...
                <td className="py-3.5 text-right">
                  <button
                    ...
                    className="zea-company-actions-trigger inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-600 transition hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700"
                    aria-label={`Open actions for ${c.name}`}
                    aria-haspopup="menu"
                    aria-expanded={actionMenu?.company.id === c.id}
                  >
      ...
      {actionMenu && createPortal(
        <>
          ...
          <div
            role="menu"
            aria-label={`Actions for ${actionMenu.company.name}`}
            className="zea-company-actions-menu fixed z-[9999] w-40 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-2xl"
          ...
          >
            <button
              ...
              onClick={() => {
                setActionMenu(null);
                setSelectedCompanyId(actionMenu.company.id);
              }}
            ...
            <button
              ...
              role="menuitem"
              onClick={() => {
                const companyId = actionMenu.company.id;
                setActionMenu(null);
                void openCompanyEditor(companyId);
              }}
              className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-amber-50"
            >
              Edit
            ...
            <button
              ...
              role="menuitem"
              onClick={() => {
                const company = actionMenu.company;
                setActionMenu(null);
      ...
      {editingCompany && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-xs">
          <form onSubmit={handleUpdateCompany} className="zea-company-edit-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="zea-company-edit-header flex items-start justify-between border-b border-slate-100 pb-3">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="zea-company-create-modal bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="zea-company-create-header px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div>
                <h3 className="font-extrabold text-slate-800 text-sm tracking-tight">New Client Company</h3>
            ...
            <form onSubmit={handleCreateCompany} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="space-y-4 text-xs font-semibold">
                <div>
                    <h4 className="zea-company-form-section-title text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2.5">Company Identity</h4>
                ...
                {/* 1. Contact Info */}
                <div>
                    <h4 className="zea-company-form-section-title text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2.5">Primary Contact</h4>
                ...
                {/* 2. Business Details */}
                <div>
                    <h4 className="zea-company-form-section-title text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2.5">Business Profile</h4>
                  <div>
                ...
                <div>
                    <h4 className="zea-company-form-section-title text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2.5">Location & Settings</h4>
                  <div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="zea-credit-ledger bg-white rounded-2xl border border-slate-200 p-6 shadow-sm lg:col-span-2 md:p-8">
          <div className="border-b border-slate-200 pb-6">
            <h3 className="text-2xl font-black tracking-tight text-slate-900 md:text-3xl">Manual Credits Adjustment Ledger</h3>
            <p className="mt-2 text-sm font-medium text-slate-500 md:text-base">Manage and adjust credits in your ledger.</p>
          </div>

          {successMsg && (
            <div className="mt-6 rounded-lg border border-emerald-100 bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">
              {successMsg}
            </div>
          )}
          {error && <div className="mt-6 rounded-lg border border-red-100 bg-red-50 p-3 text-xs font-semibold text-red-700">{error}</div>}

          <div className="border-b border-slate-200 py-8">
            <span className="block text-sm font-semibold text-slate-500 md:text-base">Ledger Balance</span>
            <span className="mt-3 block font-mono text-4xl font-black tracking-tight text-amber-500 md:text-5xl">
              ₹{balance.toLocaleString()}
            </span>
          </div>

          <div className="pt-7">
            <h4 className="text-base font-bold text-slate-900 md:text-lg">Adjust Credits</h4>
            <div className="mt-5 flex flex-col gap-4 lg:flex-row lg:items-stretch">
              <label className="relative block w-full lg:max-w-[320px]">
                <span className="sr-only">Credit adjustment amount</span>
                <span className="pointer-events-none absolute inset-y-0 left-5 flex items-center text-lg font-bold text-slate-700">₹</span>
                <input
                  type="number"
                  value={adjustAmount}
                  onChange={(e) => setAdjustAmount(e.target.value)}
                  className="h-14 w-full rounded-xl border border-slate-300 bg-white pl-12 pr-4 text-lg font-semibold text-slate-900 outline-none"
                />
              </label>
              <button
                type="button"
                onClick={() => adjustCredits('add')}
                className="flex h-14 min-w-[210px] items-center justify-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-7 text-base font-bold text-emerald-700 transition hover:bg-emerald-100"
              >
              ...
              <button
                ...
                className="flex h-14 min-w-[210px] items-center justify-center gap-3 rounded-xl border border-red-200 bg-red-50 px-7 text-base font-bold text-red-700 transition hover:bg-red-100"
              >
...
function UsersListView() {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setIsCreateDrawerOpen(false);
    };
    ...
    <div className="zea-company-user-directory space-y-6">
        ...
        <div className="overflow-x-auto text-sm">
          <table className="w-full text-left">
            <thead><tr className="border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-[9px]">
              ...
              <th className="pb-2">Status</th><th className="pb-2">Last Active</th><th className="pb-2 text-right"><span className="sr-only">Actions</span></th>
            </tr></thead>
            <tbody className="divide-y divide-slate-100 font-semibold">
                ...
                <tr key={user.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-2.5 text-right">
                    <button
                      ...
                      className="zea-user-actions-trigger inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-600 transition hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700"
                      aria-label={`Open actions for ${user.fullName}`}
                      aria-haspopup="menu"
                      aria-expanded={userActionMenu?.user.id === user.id}
                    >
                      <EllipsisVertical className="h-5 w-5" />
                    </button>
                  </td>
      ...
      {userActionMenu && createPortal(
        <>
          ...
          <div
            role="menu"
            aria-label={`Actions for ${userActionMenu.user.fullName}`}
            className="zea-user-actions-menu fixed z-[9999] w-40 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-2xl"
            style={{ top: userActionMenu.top, left: userActionMenu.left }}
          >
            <button
              type="button"
              role="menuitem"
      ...
      {isCreateDrawerOpen && (
        <div className="fixed inset-0 z-[100]">
          <button
            ...
            aria-label="Close new user panel"
          ...
          <aside
            ...
            className="zea-user-create-drawer absolute inset-y-0 right-0 z-[101] w-full max-w-md animate-in slide-in-from-right duration-300"
          >
              ...
              <div className="zea-user-create-drawer-body flex-1 space-y-5 overflow-y-auto px-6 py-6 text-sm font-semibold">
                ...
                <div>
                  ...
                  <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g. John Doe"
                    className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800 outline-none transition focus:border-indigo-500 focus:bg-white" />
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] font-bold uppercase text-slate-500">Email (Login ID)</label>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="user@company.com"
                    className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-slate-800 outline-none transition focus:border-indigo-500 focus:bg-white" />
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] font-bold uppercase text-slate-500">Login Password</label>
                ...
                <div>
                  <label className="mb-1.5 block text-[10px] font-bold uppercase text-slate-500">Account Role</label>
                  <select value={userRole} onChange={(e) => setUserRole(e.target.value as typeof userRole)}
                    className="w-full cursor-pointer rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-bold text-slate-800 outline-none">
                    <option value="COMPANY_DEVELOPER">Company Developer</option>
                    <option value="COMPANY_USER">Company User</option>
                  </select>
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] font-bold uppercase text-slate-500">Assign Company</label>
...
function VoiceProvidersView() {
  ...
  return (
    <div className="space-y-6">
        ...
        {providers.map((p) => (
          <div key={p.id} className="zea-provider-card bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col justify-between overflow-hidden hover:shadow-md transition duration-200">
            <div className="min-h-0">
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="zea-provider-actions-trigger inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-600 transition hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700"
                    aria-label={`Open actions for ${p.name}`}
                    aria-haspopup="menu"
                    aria-expanded={providerActionMenu?.provider.id === p.id}
                  >
              ...
              <div className="zea-provider-metrics mt-4 grid grid-cols-2 rounded-lg border border-slate-200">
                <div className="zea-provider-metric border-r border-slate-200 px-3 py-2.5">
                  <div className="flex items-center gap-1.5">
                    <Clock3 className="h-3.5 w-3.5 shrink-0 text-slate-500" />
                    <span className="text-[10px] font-bold text-slate-700">Latency</span>
                  </div>
                  <span className="mt-1 block text-[11px] font-medium text-slate-600">
                    {p.latencyMs === null ? 'Not measured' : `${p.latencyMs} ms`}
                  </span>
                </div>
                <div className="zea-provider-metric px-3 py-2.5">
                  <div className="flex items-center gap-1.5">
                    <Box className="h-3.5 w-3.5 shrink-0 text-slate-500" />
                    <span className="text-[10px] font-bold text-slate-700">Models</span>
                  </div>
                  <span className="mt-1 block text-[11px] font-medium text-slate-600">{p.modelCount.toLocaleString()}</span>
                </div>
              </div>
            </div>
      ...
      {providerActionMenu && createPortal(
        <>
          <button
            type="button"
            className="fixed inset-0 z-[9998] cursor-default bg-transparent"
            aria-label="Close provider actions"
            onClick={() => setProviderActionMenu(null)}
          />
          <div
            role="menu"
            aria-label={`Actions for ${providerActionMenu.provider.name}`}
            className="zea-provider-actions-menu fixed z-[9999] w-42 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-2xl"
            style={{ top: providerActionMenu.top, left: providerActionMenu.left }}
          >
            <button
              type="button"
              role="menuitem"
              onClick={() => {
                const provider = providerActionMenu.provider;
                setProviderActionMenu(null);
                void openModelManager(provider);
              }}
              className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-amber-50"
            >
              Models
            ...
            <button
              ...
              className="flex w-full items-center rounded-lg px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-amber-50"
            >
              {providerActionMenu.provider.status === 'connected' ? 'Disconnect' : 'Connect'}
            </button>
            <button
              type="button"
              role="menuitem"
...
function PhoneNumbersView() {
  ...
  useEffect(() => {
    if (!showTelephonyDrawer && !showRoutingDrawer) return;
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        setShowTelephonyDrawer(false);
        setShowRoutingDrawer(false);
      }
    };
    window.addEventListener('keydown', closeOnEscape);
    return () => window.removeEventListener('keydown', closeOnEscape);
  }, [showRoutingDrawer, showTelephonyDrawer]);
  ...
  return (
    <div className="space-y-6">
      {/* Sub tabs header */}
      <div className="zea-phone-tabs-header flex items-end justify-between gap-4 border-b border-slate-200">
        <div className="flex min-w-0 overflow-x-auto">
          <button
          onClick={() => setActiveSubTab('telephony')}
          className={`zea-phone-tab px-4 py-2.5 font-bold text-sm tracking-tight transition border-b-2 cursor-pointer flex items-center space-x-1.5 ${
            activeSubTab === 'telephony'
              ? 'zea-phone-tab-active border-indigo-600 text-indigo-600 bg-indigo-50/50 rounded-t-lg'
        ...
        {activeSubTab === 'assign' && (
          <button
            ...
            onClick={() => {
              ...
              setShowRoutingDrawer(true);
            }}
            className="mb-2 inline-flex shrink-0 items-center gap-2 rounded-lg bg-[#dfa822] px-4 py-2.5 text-xs font-bold text-black shadow-md transition hover:bg-[#e8b83f]"
          >
            <Plus className="h-4 w-4" />
            <span>Map DID Trunk Routing</span>
          </button>
        )}
      </div>
      ...
      {activeSubTab === 'telephony' ? (
                    ...
                    {numbers.map(num => (
                      <tr key={num.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-2.5 text-right">
                          {num.companyId && (
                            <TableActionsMenu
                              ariaLabel={`Actions for ${num.number}`}
                              actions={[{
                                label: 'Release Trunk',
        ...
        <>
          ...
          {showRoutingDrawer && createPortal(
            <div className="fixed inset-0 z-[100]">
              <button
                type="button"
                className="absolute inset-0 h-full w-full cursor-default bg-slate-950/55 backdrop-blur-[2px]"
                aria-label="Close map DID trunk routing panel"
                onClick={() => setShowRoutingDrawer(false)}
              />
              <aside
                role="dialog"
                aria-modal="true"
                aria-labelledby="map-did-routing-title"
                className="zea-telephony-create-drawer absolute inset-y-0 right-0 z-[101] flex w-full max-w-xl flex-col animate-in slide-in-from-right duration-300"
              >
                <div className="zea-telephony-create-header flex shrink-0 items-start justify-between border-b border-slate-100 px-6 py-5">
                  <div>
                    <h3 id="map-did-routing-title" className="text-base font-black text-slate-800 tracking-tight">Map DID Trunk Routing</h3>
                    <p className="mt-1 text-xs font-medium text-slate-400">Assign leased carrier lines to active client tenants.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setShowRoutingDrawer(false)}
                ...
                <div className="zea-telephony-create-body flex-1 overflow-y-auto px-6 py-6">
                  ...
                  <form onSubmit={handleAssignNumber} className="zea-telephony-create-form space-y-4 text-xs font-semibold">
                    <button
                      type="button"
                    ...
                    <div>
                      ...
                      >
                        ...
                        {assignableNumbers.map(n => (
                          <option key={n.id} value={n.id}>{n.number} (Unassigned Pool)</option>
                        ))}
                      </select>
                    </div>
