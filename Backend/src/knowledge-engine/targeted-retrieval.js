...
function buildEnrichedSparseQuery(queryContext, recordScope) {
  ...
  const base = queryContext?.sparseText ?? '';
  // Append alias/STT forms from publication records whose canonical name
  // appears in the query. This handles synonym expansion (e.g. "rate" → "price")
  // without any external service — forms are already indexed at publication time.
  const baseTokenSet = new Set(tokens(base));
  const extra = [];
  for (const record of recordScope.values()) {
    const nameTokens = tokens(record.canonicalName ?? '');
    if (!nameTokens.length) continue;
    const overlap = nameTokens.filter((t) => baseTokenSet.has(t)).length;
    if (overlap === 0) continue;
    // Add STT and phonetic variants as extra tokens so BM25 can match them.
    for (const form of record.searchForms ?? []) {
      extra.push(...tokens(form));
    }
  }
  if (!extra.length) return base;
  return `${base} ${[...new Set(extra)].join(' ')}`.trim();
}

function bm25Candidates(
  input, sparseIndexes, scope, allowedTypes, limit, queryContext, recordScope,
) {
  // Enrich the query with alias/STT forms from records whose canonical name
  // overlaps with the current utterance. This improves recall for synonym
  // queries like "rate", "cost", "evlo" that don't share exact tokens with
  // the canonical catalog item name.
  const enrichedSparseText = buildEnrichedSparseQuery(queryContext, recordScope);
  const queryTokens = [...new Set(tokens(enrichedSparseText || queryContext?.sparseText || input.utterance))];
  ...
  for (const [namespace, namespaceAllowedTypes] of Object.entries(groups)) {
    ...
    const ranked = documents.flatMap((document) => {
      ...
      // Reject candidates that barely matched — a single weak token cannot
      // outrank a well-corroborated Qdrant or structured result in RRF.
      if (!matched || score < BM25_MIN_SCORE) return [];
      return [{
        tenantId: input.tenantId,
        agentId: input.agentId,
...
async function semanticCandidates(
  ...
  // Qdrant performs a full cross-namespace scan regardless of what the
  // classifier narrowed down to. The classifier may decide the primary
  // namespace is FAQ, but a Catalog item can still be semantically close
  // and should surface — the reranker and RRF will discard irrelevant
  // cross-namespace hits rather than hiding them at search time.
  // Structured and BM25 remain namespace-filtered for precision.
  const allRecordTypes = new Set(
    Object.values(namespaceTypes).flat(),
  );
  const qdrantAllowedTypes = allRecordTypes;
  const namespaceGroups = selectedNamespaceTypes(qdrantAllowedTypes);
