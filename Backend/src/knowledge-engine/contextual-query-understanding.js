...
export function understandContextualKnowledgeQuery(input, resolution) {
  ...
  // Short follow-up guard: if the caller's utterance is very short (≤ 5 tokens)
  // and there is an active memory entity but no explicit entity signal, treat
  // this as a contextual follow-up. This fixes queries like "price evlo?" or
  // "cost?" after a package was selected, where no entity name appears in the
  // utterance so contextDependent would otherwise be false.
  const utteranceTokenCount = String(input.utterance ?? '').trim().split(/\s+/u).filter(Boolean).length;
  const shortFollowUp = !hasCurrentEntitySignal
    && !protocolRoute
    && Boolean(memoryEntity)
    && utteranceTokenCount > 0
    && utteranceTokenCount <= (env.CONTEXT_SHORT_FOLLOWUP_MAX_TOKENS ?? 5);
  const contextDependent = shortFollowUp || (
