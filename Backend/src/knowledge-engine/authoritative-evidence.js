...
import { rerankedCandidates } from './cross-channel-reranker.js';
...
export function fuseCandidateRankings(retrieval, {
  ...
  const hasFocusedCatalogRecord = reservations.some((reservation) => (
    String(reservation?.recordType ?? '').toUpperCase() === 'CATALOG_ITEM'
  ...
  const normalizedProviderScore = (channel, score) => (
    channel === 'bm25' ? Math.min(1, Number(score ?? 0) / 15) : Number(score ?? 0)
  );
  const strongestProviderScore = (candidate) => Math.max(
    0, ...Object.entries(candidate.providerScores ?? {}).map(([ch, sc]) => (
      normalizedProviderScore(ch, sc)
    )),
  );
...
export async function rankAndHydrateAuthoritativeEvidence({
  ...
  // Apply cross-channel quality gates after RRF fusion.
  // Single-channel weak matches and candidates with no name overlap to the
  // caller's query are rejected before PostgreSQL hydration. Reserved records
  // (explicit entity, canonical memory, comparisons) bypass all gates.
  const sparseQueryText = retrieval?.queryContext?.sparseText ?? input?.utterance ?? '';
  const reranked = rerankedCandidates(fusion.candidates, fusion, sparseQueryText);
  const rerankedFusion = Object.freeze({
    ...fusion,
    candidates: reranked.accepted,
  });
  const contextRunner = dependencies.contextRunner ?? withTenantContext;
  let rows = [];
  if (rerankedFusion.candidates.length) {
    const requested = rerankedFusion.candidates.map((candidate) => ({
  ...
  const fusedById = new Map(rerankedFusion.candidates.map((candidate) => (
  ...
  const rejectedRecordIds = rerankedFusion.candidates
  ...
  const selectedCandidateWasRanked = rerankedFusion.candidates.some((candidate) => (
  ...
  if (rerankedFusion.candidates.length > 0 && evidence.length === 0) {
    throw new AppError(503,
      ...
      'KNOWLEDGE_AUTHORITATIVE_HYDRATION_EMPTY', {
        stage: 'authoritative_hydration',
        selectedCandidates: rerankedFusion.candidates.map((candidate) => ({
          recordId: candidate.recordId,
          recordType: candidate.recordType,
          knowledgeBaseId: candidate.knowledgeBaseId,
  ...
  return Object.freeze({
    ...
    agentId,
    callId: input.callId,
    fusion: rerankedFusion,
    evidence: verifiedRecords,
    verifiedRecords,
    ambiguity,
    conflict,
    rejectedRecordIds: Object.freeze(rejectedRecordIds),
    rerankerRejectedWeakSingleChannel: reranked.rejectedWeakSingleChannel,
    rerankerRejectedNoNameOverlap: reranked.rejectedNoNameOverlap,
