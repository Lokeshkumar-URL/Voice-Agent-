...
export function resolveCommonTtsConfiguration(providerConfig) {
  ...
  const rawVoiceId = setting(settings, 'voiceId', 'ttsVoiceId')
...
export async function requireAudioResponse(response, providerConfig) {
  ...
  console.error(`🚨 TTS PROVIDER ERROR (${providerConfig.providerId || 'TTS'}): status=${response.status} message=${details}`);
