...
const envSchema = z.object({
  ...
  VOICE_TTS_FIRST_AUDIO_TIMEOUT_MS: z.coerce.number().int().min(250).max(5000).default(2500),
  ...
  // Maximum token count in a caller's utterance to be treated as a short
  // contextual follow-up (e.g. "price evlo?" = 2 tokens). When the utterance
  // is this short and an active memory entity exists, contextDependent is
  // forced to true so the selected entity is included in retrieval.
  CONTEXT_SHORT_FOLLOWUP_MAX_TOKENS: z.coerce.number().int().min(1).max(20).default(5),
  ...
  QDRANT_COLLECTION_PREFIX: z.string().regex(/^[a-z0-9_]+$/)
    .default('zea_voice_company_e5_small_v1'),
