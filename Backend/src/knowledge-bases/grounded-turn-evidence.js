...
export function buildGroundedLlmInput({
  ...
  // LLM input budget guard: drop oldest conversation history turns until the
  // estimated token count (chars / 4) fits within VOICE_LLM_PROMPT_BUDGET_TOKENS.
  // Evidence records and the current question are never trimmed — only history.
  let recentRelevantTurns = compactRelevantTurns(input);
  const promptBudgetTokens = env.VOICE_LLM_PROMPT_BUDGET_TOKENS;
  const estimatedTokens = (turns) => {
    const historyChars = turns.reduce((sum, t) => sum + (t.content?.length ?? 0), 0);
    const evidenceChars = hydratedRecords.reduce(
      (sum, r) => sum + (r.content?.length ?? 0), 0,
    );
    return Math.ceil((historyChars + evidenceChars) / 4);
  };
  while (recentRelevantTurns.length > 1
    && estimatedTokens(recentRelevantTurns) > promptBudgetTokens) {
    // Drop the oldest turn (index 0) first to preserve recent context.
    recentRelevantTurns = Object.freeze(recentRelevantTurns.slice(1));
  }
