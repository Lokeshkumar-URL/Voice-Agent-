...
function parseCatalog(extraction) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // Guard against configuration-key strings being parsed as item names.
    // Lines like "RATE_PER_SQ_FT : 45" should be attributes or ignored;
    // they must not become separate catalog items with a key-looking name.
    const keyPatternRegex = /^[A-Z][A-Z0-9_]{2,}$/u;
    if (keyPatternRegex.test(name.trim())) {
      warnings.push(
        `Catalog line ${index + 1} has a name that looks like a configuration key ("${name}"); `
        + 'use ATTRIBUTES or an explicit ITEM directive if this is intentional',
      );
      continue;
    }
    // Items without at least a price, a description, or an explicit ITEM
    // directive are almost certainly parser noise (e.g. a heading line
    // that matched the price regex by accident).
    const hasMinimumContent = metadata.explicitItem
      || metadata.price !== null
      || parsedPrice !== null
      || Boolean(metadata.description)
      || Boolean(metadata.itemKey ?? metadata.key);
    if (!hasMinimumContent) {
      warnings.push(
        `Catalog line ${index + 1} ("${name}") was skipped — no PRICE, DESCRIPTION or explicit ITEM directive`,
      );
      continue;
    }
...
function parseGeneralKnowledge(extraction) {
  ...
  const domainKeywordEnrichment = [
    ...
    { terms: [/zea ai/i, /zea brain/i, /zea voice/i, /zeacrm/i, /zea play/i], aliasText: 'Tamil Queries: ஜீ ஏஐ | ஜீ பிரைன் | ஜீ வாய்ஸ் | ஜியா சிஆர்எம் | ஜீ ப்ளே | சியராம் | ஏஐ' },
    { terms: [/ai/i, /ai calling/i, /voice/i, /calling/i], aliasText: 'Tamil Queries: ஏஐ | ஏ.ஐ | AI | AI calling | Zea Voice' },
