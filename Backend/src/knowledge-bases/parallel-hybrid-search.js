...
function scopedPublicationRecord(reservation, bundles = []) {
  for (const bundle of bundles) {
    ...
    // Authoritative identity reservations (explicit entity, current entity selected
    // by the caller) use score 1.0 — they are exact identity matches, not
    // similarity scores and should always lead RRF.
    // Use-case / published-use-case reservations use 0.9 so that a Qdrant
    // result with stronger semantic similarity can still outrank a weak
    // structured match that came from a single keyword overlap.
    const authoritativeReasons = new Set([
      'explicit_entity', 'explicit_current_entity', 'canonical_memory',
      'latest_request_record', 'explicit_comparison',
    ]);
    const score = authoritativeReasons.has(String(reservation.reason ?? '').toLowerCase())
      ? 1 : 0.9;
    return Object.freeze({
      ...candidate,
      canonicalIdentity,
      canonicalIdentityKey: canonicalRecordIdentityKey(candidate),
      channel: 'structured', rank: 1, score,
