...
for (let pass = 1; pass <= repeats; pass += 1) {
  for (const tenant of tenants) {
    try {
      ...
      assert.ok(naturalSource, `[Industry: ${tenant.industry}] Natural semantic question (${tenant.naturalQuestion}) must hydrate ${first.record_id}, got: ${JSON.stringify(naturalEnvelope.sources.map(s=>s.recordId))}`);
