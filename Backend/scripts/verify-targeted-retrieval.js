...
assert.deepEqual(retrieval.channels.qdrant.map((candidate) => candidate.recordId), [beta.record_id, workflow.record_id]);
...
assert.deepEqual(new Set(qdrantOptionCalls.flatMap((options) => options.recordTypes)),
  new Set(['CATALOG_ITEM', 'CATALOG_CATEGORY', 'FAQ', 'CONVERSATION_NODE', 'WORKFLOW_RULE', 'KNOWLEDGE_CHUNK']),
